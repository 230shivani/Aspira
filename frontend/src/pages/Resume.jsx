...
export default function Resume() {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>${data.name || "Resume"}_AI_CareerHub</title>
                    ...
                    <div className="space-y-6">
                      {data.projects.map((proj, index) => (
                        <div key={index} className="p-6 rounded-2xl bg-white/5 border border-white/5 space-y-4 relative group">
                          ...
                          <Input label="Project Name" value={proj.name} onChange={(v) => updateField("projects", index, "name", v)} placeholder="AI CareerHub AI" />
                          <div className="grid grid-cols-2 gap-4">
